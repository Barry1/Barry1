...
```sh
sed --in-place=.sedbackup --regexp-extended 's/(.*- python@)(.*)/\1>=3.12.0/g' trunk.yaml 
```

If you want to do that recursivly on every `.trunk/trunk.yaml` file, the following command might help:

```sh
find . -wholename "*/.trunk/trunk.yaml" -not -path "*.venv*" -print -execdir sed --in-place=.sedbackup --regexp-extended 's/(.*- python@)(.*)/\1>=3.11.0/g' trunk.yaml \{\} \;
```
