#!/bin/bash
###############################################################################
# best use by putting `source /home/ebeling/GitHub/Barry1/LinuxSetup/my.bashrc`
# in your .bashrc
###############################################################################
...
find() {
    start=${1:-.}
    shift 2>/dev/null
    command find "$start" -not -path \*\.venv\* -not -path \*\.tox\* -not -path \*\.cache\* -not -path \*\.asv\* "$@"
}
#export -f find
### make wrapper to put common rulesin $HOME/.Makefile
make() {
